...
def generar_cuadro_maestro(mes, ano, h_cont, h_est, sugerencias):
    ...
    # Marcadores de Nómina e Historial
    ...
    for d in range(1, dias_mes + 1):
        ...
        # C. SUGERENCIAS (Mandan sobre el resto)
        for p in PERSONAL_OFICIAL:
        ...
        # D. REPARTO MATEMÁTICO (Cuotas y Espaciado)
    ...
    # CÁLCULOS FINALES
    df['TOTAL MES (1-31)'] = df.apply(lambda r: sum(1 for x in r if any(t in str(x) for t in ['C', 'N']) and 'P' not in str(x)), axis=1)
...
# --- 4. INTERFAZ ---
st.title("🏥 Optimizador de Turnos V3 - Nómina 21-20")
# ... (Interfaz Streamlit igual que la anterior)
