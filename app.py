...
with st.sidebar:
    ...
    link_sheet = st.text_input("Link de Sugerencias:", "https://docs.google.com/spreadsheets/d/1PZwvv0XQtSEDfC5GO6OlG7Fn8HqJNQUBZ1RNSRgBsss/edit?gid=0#gid=0")
    st.header("2. Configuración")
    mes_n = st.selectbox("Mes a Proyectar", range(1, 13), index=datetime.now().month - 1)
...
if st.button("🚀 GENERAR CUADRO", type="primary", use_container_width=True):
    ...
    st.dataframe(resultado.style.applymap(aplicar_colores, subset=[c for c in resultado.columns if c.isdigit()]), use_container_width=True)
    output = io.BytesIO()
    with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
        resultado.to_excel(writer, index=True)
    st.download_button("📥 Descargar Excel", output.getvalue(), f"Cuadro_{mes_n}.xlsx", use_container_width=True)
